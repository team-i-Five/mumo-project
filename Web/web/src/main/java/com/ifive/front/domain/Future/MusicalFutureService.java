...
public interface MusicalFutureService {
    // ID값 파라미터로 넣어서 호출하면, ML서버에 접속해서 ID값으로 유사한 뮤지컬 5개의 ID값을 가져오고
    // 해당 ID값으로 DB를 조회해서 MusicalFuture 정보를 가져와서 DTO에 담습니다.
    List<MusicalFutureDTO> getFutureDTOsbyIdFromML(String id);
    ...
    List<MusicalFutureDTO> getMusicalsByIds(List<Integer> musicalIds);
